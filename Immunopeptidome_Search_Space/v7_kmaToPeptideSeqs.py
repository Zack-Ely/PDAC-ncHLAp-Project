#----------------
#note, this script has been modified from Smart and Margolis et al 2018. 
#also note IRFinder was used as a separate RI predictor
...
def createUniqueIntronList(csvfile, outpath):
	...
	chromlocs = np.loadtxt(csvfile, dtype=str, delimiter=',', skiprows=1, usecols=[2,3])
...
def getSeqs(intronlocs, tpms, nAAs, outpath):
	errfile = open(outpath + '/introns_skips.txt', 'a')
	outfile = open(outpath + '/CodingIntronSeqsFASTA.txt','a')
	headermapfile = open(outpath + '/Intronheadermap.txt','a')
	ex_int_conflict_skips=0
	no_gene_skips=0
	no_transl_skips=0
	for l in range(0, len(intronlocs)):
		...
		fullcommand = 'mysql -h genome-euro-mysql.soe.ucsc.edu -u genome -D hg19 -A --connect_timeout=60 -e "'+sqlcommand+'"'
		...
		if not len(exonstarts) == len(exonends) == len(exonframes):
			ex_int_conflict_skips=ex_int_conflict_skips+1
			continue
		...
		if strand == ('+'):
			...
			if frame == 0:
				...
				no_transl_skips=no_transl_skips+1
				continue
		else: 
		...
		#need to validate this 1-based change (compared to 3_kma...py)
		one_base = wholeseqstart + 1
		location = chrom + ':' + str(one_base) + '-' + str(wholeseqend)
		loc = '-seq='+loc
		twobitoutput = (subprocess.check_output(['twoBitToFa',
                        loc,'/path/intron_pipeline/tools/bits/hg19.2bit','stdout']))
                # Parse output
		...
		##blocking the below in v5 script
		...
		#if len(peptide) < nAAs:
		#	continue
		strand_newheaderline = '>'+location+'|'+strand
		newheaderline = '>'+location
                outfile.write(newheaderline+'\n')
                outfile.write(sequence+'\n')
		headermapfile.write(strand_newheaderline+'\t'+headerline+'\n')
	errfile.write(str(ex_int_conflict_skips)+'\n')
	errfile.write(str(no_gene_skips)+'\n')
	errfile.write(str(no_transl_skips)+'\n')
